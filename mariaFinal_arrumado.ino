...
#define head_pin 7
#define leftDCPin1 37
#define rightDCPin1 33
#define leftDCPin2 39
#define rightDCPin2 35
#define leftDCPwm 6
#define rightDCPwm 11
...
int adjustIRS(int x){ /*return (x+30>180)?180:x+30;*/ return x;}
...
void setup() {
  ...
  delay(4000);//initializing motors. plenty of delay to let capacitors stabilize */
  Serial.begin(115200);
  delay(200);
  ...
  leds.show();
  analogWrite(leftDCPwm, 255);
  analogWrite(rightDCPwm, 255);
  leftStop();
  rightStop();
  Serial.println('z');
}
...
void selfDestroy(){
  ...
  for(int x = 0; x > -160 && a;){
    for (int i = 0; i<=LED_COUNT &&a; i++)//makes an yellow circle
    {
      ...
      leds.show();
      x++;
      if(x >= 50*5.8 && a){ XBee.write('D'); a = false;}//orders the secondary robot to leave
    }
  }
  delay(3000);
  for(byte i = 1; i <= 8; ++i){
...
void moveLeft(byte Pin1, byte Pin2, byte velocity){
  analogWrite(leftDCPwm, velocity);
  digitalWrite(leftDCPin1, Pin1);
  digitalWrite(leftDCPin2, Pin2); 
}
void leftFowards(byte velocity){
  moveLeft(1, 0, velocity);
}
void leftFowards(){
  leftFowards(150);
}
void leftBackwards(byte velocity){
  moveLeft(0, 1, velocity);
}
void leftBackwards(){
  leftBackwards(150);
}
...
void moveRight(byte Pin1, byte Pin2, byte velocity){
  analogWrite(rightDCPwm, velocity);
  digitalWrite(rightDCPin1, Pin1);
  digitalWrite(rightDCPin2, Pin2); 
}
void rightFowards(byte velocity){
  moveRight(1, 0, velocity);
}
void rightFowards(){
  rightFowards(150);
}
void rightBackwards(byte velocity){
  moveRight(0, 1, velocity);
}
void rightBackwards(){
  rightBackwards(150);
}
